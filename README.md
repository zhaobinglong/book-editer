...
0.  gray        灰色 + 灰色的背景图,   主编辑区上下不透明
1.  deepGray    深灰色 + 深灰色背景图,  主编辑区上下不透明
2.  paper,      paper,              主编辑区上下工具栏透明
3.  metal,      metal,              主编辑区仅下工具栏透明(不透明，使用什么颜色)
4.  water,      water,              主编辑区仅上工具栏透明
5.  falling,    falling,            主编辑区上下工具栏不透明

补充说明：
主编辑区透明部分不应用自定义颜色；非透明及缩略图，搜索、目录及下载、分享、电话、邮件都将使用自定义颜色;
点击颜色选择器时，弹出默认导航;
✅删除主编辑区的图片，主编辑区显示  background的颜色;
✅修改自定义颜色后，如果再次点击皮肤将恢复所有皮肤元素，覆盖自定义；
如果当前有改变背景图，则再选择皮肤时，提供是否保留背景的选项；
1. 颜色自定义覆盖皮肤；（一旦设置一次颜色值，再去更改皮肤就不会起作用了，因为自定义的颜色值是一直存在的，没有删除颜色值选项）
2. 皮肤的工具栏透明，有四种组合；
3. 应用到弹窗，包括手机；类似手机端的缩略图;
