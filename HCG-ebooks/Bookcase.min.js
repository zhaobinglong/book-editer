	...
	bindEvents: function() {
		...
		if (hf_utils.isLteIE8() ? this.$content.on(hf_utils.bindClcik(), ".book-wrapper", function() {
			...
			if (!e.hasClass("book-down") && (e.hasClass("book-wrapper") || f.length > 0)) {
				...
				// 	break;
				// default:
				// 	new BookDetailPanel(e.clone(), h, i, c)
				// }
			}
		...
		hf_utils.isLteIE8() ? this.$content.on(hf_utils.bindClcik(), ".case-list-title", function() {
			var a = $(this).attr("data-index"),
				e = b.bookObj[a],
				f = ("/" + c.domain + "." + c.domainSuffix + "/", e.url.replace(/^https?:/, "https:") + "index.html");
